...
Connects to the Mosquitto broker on the board and lets you:
  - Monitor ALL MQTT topics in real-time
  - Send every command type (switch, dimmer, RGB, tunable, HVAC, fan, curtain, scene)
  - Test ACL and auth (port 1884)
  - Stress-test with rapid-fire commands
  - Subscribe to individual topics
  - View retained messages
  - Follow the full 4-step command cycle

Usage:
    python3 okas_mqtt_tester.py

Requires: paho-mqtt, tkinter (built-in)
Install deps: pip3 install paho-mqtt
"""
...
# Topic constants (mirrors the system)
...
DTO = lambda x: x.strftime("%H:%M:%S.%f")[:-3] if hasattr(x, "strftime") else datetime.now().strftime("%H:%M:%S.%f")[:-3]
...
class MqttTesterGui:
    def __init__(self):
        ...
        # MQTT state
        self.local_client = None   # internal broker (1883)
        self.auth_client = None    # auth broker (1884)
    ...
    def _build_ui(self):
        ...
        style.map("Green.TButton", background=[("active", "#b8f2b3")])
        ...
        # Loads frame
        loads_frame = ttk.LabelFrame(left, text="Loads (click to send test command)", padding=8)
        ...
        # Command row 2 — parameters as key=value pairs
        ...
        # Quick-buttons for common params
        ...
        # Command cycle — full 4-step
        cycle_frame = ttk.LabelFrame(right, text="Full Command Cycle", padding=8)
        cycle_frame.pack(fill=tk.X, pady=(0, 6))

        ttk.Label(cycle_frame, text="Step through the full 4-step cycle:").pack(anchor=tk.W)
        ...
        # Theme overrides for tk widgets
        for w in (self.root,):
            w.tk_setPalette(background=bg, foreground=fg, selectBackground=accent, selectForeground=bg)
    ...
    def _run_cycle(self):
        """Publish a command and auto-subscribe to the ack/status flow."""
        try:
        ...
        # Subscribe to relevant topics to capture the cycle
    ...
    def _clear_retained(self):
        """Publish empty retained messages to clear old state."""
    ...
    def _on_load_double_click(self, event):
        """Double-click a load to auto-fill the command panel."""
        ...
        ld_id = item["values"][0]
        typ = item["values"][2]
        # Derive a sensible test command based on type
        default_params = {
            "swt": '{"swt":true}',
            "dim": '{"bri":50}',
    ...
    def _update_load_state(self, topic, payload_str):
        """Update the state column for a load when status/{id} is received."""
        try:
    ...
    def _log_monitor(self, label, message):
        ...
        if label.startswith("[local]"):
            pass  # default
        if "ERR" in label or "Error" in message or "FAILED" in message:
            self.monitor_text.tag_add("err", f"end-2l", "end-1l")
            self.monitor_text.tag_config("err", foreground="#f38ba8")
        elif "CONNECTED" in message or "connected ✓" in message:
