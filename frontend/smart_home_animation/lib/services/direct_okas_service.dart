// lib/services/direct_okas_service.dart
import 'dart:convert';
...
class DirectOKASService extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    // Setup MDNS callbacks with MAC address
    _mdns.addListener((ip, {String? mac}) {
  ...
  Future<bool> connect() async {
    ...
    try {
      ...
      print('🔌 Connecting to OKAS device at: $url');

      final response = await http.get(url).timeout(const Duration(seconds: 5));

      if (response.statusCode == 200) {
        _isConnected = true;
        _deviceIp = Constants.currentIp;

        // Try to get MAC from mDNS if not already set
        // if (_deviceMac == null && _deviceIp != null) {
        //   _deviceMac = await _mdns.getMacAddress(_deviceIp!);
        // }

        await loadRoomsAndDevices();
        print('✅ Connected to OKAS device at ${Constants.localHost}');
        if (_deviceMac != null) {
          print('   📡 Board MAC: $_deviceMac');
        }
        return true;
      } else {
        _error = 'Device returned ${response.statusCode}';
        _isConnected = false;
        return false;
      }
    } catch (e) {
  ...
  // Get board information including MAC
  Map<String, dynamic> getBoardInfo() {
    return {
      'ip': _deviceIp,
      'mac': _deviceMac,
